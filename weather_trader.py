...
CONFIG_SCHEMA = {
    ...
    "order_type":        {"env": "SIMMER_WEATHER_ORDER_TYPE",        "default": "FAK", "type": str,
                          "help": "Order type for Polymarket trades: FAK (default, recommended for bots), GTC (rest until filled), FOK (fill all or cancel), GTD (fill until date)"},
}
...
ORDER_TYPE = _config["order_type"].upper() if _config["order_type"] else "FAK"
if ORDER_TYPE not in _VALID_ORDER_TYPES:
    raise ValueError(f"Invalid SIMMER_WEATHER_ORDER_TYPE '{ORDER_TYPE}'. Must be one of: {_VALID_ORDER_TYPES}")
...
def execute_trade(market_id: str, side: str, amount: float) -> dict:
    ...
    try:
        result = get_client().trade(
            ...
            order_type=ORDER_TYPE, source=TRADE_SOURCE, skill_slug=SKILL_SLUG,
        )
        return {
...
def execute_sell(market_id: str, shares: float) -> dict:
    ...
    try:
        result = get_client().trade(
            ...
            shares=shares, order_type=ORDER_TYPE, source=TRADE_SOURCE, skill_slug=SKILL_SLUG,
        )
        return {
            "success": result.success, "trade_id": result.trade_id,
            "error": result.error, "simulated": result.simulated,
...
def run_weather_strategy(dry_run: bool = True, positions_only: bool = False,
    ...
    # Pre-flight balance check for live mode
    if not dry_run:
        portfolio = get_portfolio()
        balance = portfolio.get("balance_usdc", 0) if portfolio else 0
        wallet_address = os.environ.get("WALLET_ADDRESS", "unknown")
        if balance == 0:
            log(f"\n❌ Pre-flight check failed:")
            log(f"   USDC.e balance: $0.00")
            if wallet_address != "unknown":
                log(f"   Wallet: {wallet_address}")
            log(f"")
            log(f"   Your Polymarket wallet needs USDC.e (not native USDC).")
            log(f"   Bridge guide: https://simmer.markets/docs/usdc-bridge")
            log(f"")
            log(f"   Or run in paper mode first: python weather_trader.py (no --live)")
            sys.exit(1)
