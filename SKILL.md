...
- **Fixed order type defaults**: Default changed from GTC to FAK (recommended for bots). All 4 Polymarket order types now supported: FAK, GTC, FOK, GTD. Invalid order type now raises a clear error at startup.
- **Pre-flight balance check**: Running with `--live` now checks USDC.e balance before attempting any trades. If balance is $0, prints a clear error with bridge guide link instead of silently failing.
- **USDC.e bridge guide**: New `docs/usdc-bridge.md` with step-by-step instructions for all bridge paths (Ethereum → Polygon, native USDC → USDC.e, cross-chain from Base/Solana/etc).

### v1.15.0

- **Order type support**: New `SIMMER_WEATHER_ORDER_TYPE` config (FAK, GTC, FOK, GTD). FAK (default, recommended for bots) fills immediately or cancels; GTC rests on the book until filled. Match to market conditions.
...
| Order type | `SIMMER_WEATHER_ORDER_TYPE` | FAK | Order type: FAK (recommended for bots, fill immediately or cancel), GTC (rest until filled), FOK (fill all or cancel), GTD (fill until date) |
...
**"Pre-flight check failed: USDC.e balance $0.00"**
- This check runs automatically before any live trades to prevent failed orders
- Polymarket uses **USDC.e** (bridged USDC, contract `0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174`) — not native USDC
- If you bridged USDC to Polygon recently, you likely received native USDC instead
- **Full bridge guide:** `docs/usdc-bridge.md` or [simmer.markets/docs/usdc-bridge](https://simmer.markets/docs/usdc-bridge)
- Quick fix: swap native USDC → USDC.e on Uniswap (Polygon network), then retry
